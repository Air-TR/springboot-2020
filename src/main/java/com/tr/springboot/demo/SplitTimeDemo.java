...
public class SplitTimeDemo {
    ...
    public static List<Long> segmentTime(LocalDateTime startTime, LocalDateTime endTime) {
        List<Long> durations = new ArrayList<>();

        // 处理 startTim e和 endTime 在不同日期的情况
        ...
        while (!currentDateTime.isAfter(endTime)) {
            ...
            if (segmentStart.isBefore(segmentEnd)) {
                ...
                durations.add(duration.toSeconds());
            }
            ...
            // 如果已经过了凌晨，将 nextDateTime 调整为正确的日期
    ...
    public static void main(String[] args) {
        LocalDateTime startTime = LocalDateTime.of(2024, 12, 18, 10, 20, 00);
        LocalDateTime endTime = LocalDateTime.of(2024, 12, 18, 10, 23, 00);

        List<Long> durations = segmentTime(startTime, endTime);

        for (Long seconds : durations) {
